...
def get_report_path(report_quarter: str) -> Path:
    ...
    """
    return Path("data/processed_faers") / report_quarter / f"MERGED{format_quarter(report_quarter)}.csv"
...
def load_report(report_quarter: str) -> pd.DataFrame:
    ...
    if report_quarter not in get_processed_quarters():
        raise ValueError(f"Report quarter {report_quarter} is not supported. Supported report quarters are: {get_processed_quarters()}")
